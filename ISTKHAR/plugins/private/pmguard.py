...
from ISTKHAR.database.pmpermitdb import get_approved_users, pm_guard
import ISTKHAR.database.pmpermitdb as ISTKHAR
...
async def pmguard(client, message):
    ...
    if not arg:
        ...
        return
    await ISTKHAR.set_limit(int(arg))
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await ISTKHAR.set_block_message(ISTKHAR.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await ISTKHAR.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await ISTKHAR.get_pm_settings()
    await ISTKHAR.allow_user(chat_id)
...
async def deny(client, message):
    chat_id = message.chat.id
    await ISTKHAR.deny_user(chat_id)
